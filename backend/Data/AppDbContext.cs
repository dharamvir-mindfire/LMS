...
public class AppDbContext : DbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        {
            // Materials is a list of {title,url} objects; SQL Server has no
            // object-array column type, so it's persisted as a JSON string,
            // same approach as Question.Options.
            entity.Property(l => l.Materials)
                .HasConversion(
                    v => JsonSerializer.Serialize(v, (JsonSerializerOptions?)null),
                    v => JsonSerializer.Deserialize<List<LessonMaterial>>(v, (JsonSerializerOptions?)null) ?? new List<LessonMaterial>())
                .Metadata.SetValueComparer(new Microsoft.EntityFrameworkCore.ChangeTracking.ValueComparer<List<LessonMaterial>>(
                    (a, b) => (a ?? new()).SequenceEqual(b ?? new()),
                    v => v.Aggregate(0, (hash, m) => HashCode.Combine(hash, m)),
                    v => v.ToList()));

            // A subject cannot be deleted while it still has lessons — mirrors
            // the 409 guard in SubjectService.
